...
/** Язык витрины по стране; экспортируется — тот же маппинг нужен gplayRpc. */
export function langOf(country: string): string {
 ...
 *
 * ПРЕДЕЛ ГЛУБИНЫ ЭТОГО ПАРСЕРА: HTML-страница /store/search после редизайна
 * 2022 отдаёт только первую «страницу» (~15-30 приложений) и пагинации в
 * разметке не имеет. Поэтому rank=null при totalResults>0 здесь значит
 * «глубже выдачи», а не «нет в сторе».
 *
 * ВАЖНО: это ограничение разметки, а не витрины. Полную выдачу (230-250
 * позиций, порядок совпадает с этим парсером) отдаёт batchexecute-RPC самой
 * витрины — см. scrapers/gplayRpc.ts. Прежний вывод «qnKhOb отвечает
 * PlayDataError» был верным наблюдением с неверным выводом: qnKhOb работает
 * только с токеном из первой страницы (RPC lGYRle), вызванный первым он
 * действительно падает. Мобильный protobuf-API (Finsky, scrapers/finsky/)
 * для обычных запросов, наоборот, отдаёт пустой список — он тут не помощник.
 */
